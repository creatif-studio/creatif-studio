  ...
  return (
    <ProductsContainer
      classNameContainer={clsx(
        className,
        "mt-24 bg-zinc-900 pt-16 pb-0 rounded-xl"
      )}
    ...
    >
      <section className="ml-7 lg:ml-10 xl:flex xl:justify-between">
        <section className="w-9/12 md:w-10/12 lg:w-7/12 lg:pr-22">
          <ProductsSectionSubtitle className="text-white text-3xl leading-tight md:font-bold lg:text-4xl xl:text-4.5xl xl:leading-snug">
            {Title}
          </ProductsSectionSubtitle>
          <ProductsSectionDescription className="text-gray-200">
            {Description}
          ...
          {Array.isArray(cardData) &&
            cardData.map(({ title, description }, i) => (
              <IntroCard className="p-0 mr-5 md:mr-5 lg:mr-5 xl:mr-5" key={i}>
                <IntroCard.Title
                  classNameOverrideBaseSize="text-base"
                  className="text-white font-bold md:text-base lg:text-base"
                >
                  {title}
                </IntroCard.Title>
                <IntroCard.Description className="text-gray-200">
                  {description}
